...
// ⚠️ تکایە ئەڤێ ل خوارێ ب App ID یێ خۆ بگەهۆڕە ⚠️
...
window.createRoom = async function() {
    ...
    try {
        ...
        await window.joinAgoraChannel(); // دەستپێکرنا چاتا دەنگی
...
window.rejoinOnlineRoom = async function() {
    ...
    try {
        ...
        if (data.hostId === window.playerId) { window.isHost = true; onDisconnect(ref(db, 'rooms/' + window.roomCode)).remove();
        }
        else { window.isHost = false;
        onDisconnect(ref(db, `rooms/${window.roomCode}/players/${window.playerId}`)).remove(); }
...
window.showWaitingScreen = function() {
    ...
    document.getElementById('waitingScreen').style.display = 'block'; document.getElementById('displayRoomCode').innerText = window.roomCode;
    document.getElementById('onlineFloatingControls').style.display = 'flex'; 
    window.setupChatAndReactions(); 
    
    if (window.isHost) { document.getElementById('hostSettingsSection').style.display = 'block';
...
window.updateHostSettings = function() {
    ...
    let val = parseInt(document.getElementById('hostMaxNumInput').value); if(isNaN(val) || val < 10) val = 10;
...
window.setPlayerReady = async function() {
    ...
    let numbers = []; inputs.forEach(inp => { let val = parseInt(inp.value); if (!isNaN(val) && val >= 1 && val <= window.maxGameNumbers) numbers.push(val); });
    if(!name || numbers.length === 0) { alert(`ناڤێ خۆ بنڤیسە!`); return;
    }
    if(numbers.length !== window.requiredNumbers) { alert(`تکایە هەمی خانەیان تژی بکە! پێدڤییە ${window.requiredNumbers} ژمارەیان بنڤیسی.`);
    return; }
    if ([...new Set(numbers)].length !== numbers.length) { alert('ژمارە دووبارە کرینە!');
    return; }
...
window.startGameHost = async function() {
    ...
    if(pArr.filter(p => !p.isReady).length > 0) { alert("هێشتا هەمیان ئامادەیی نیشان نەدایە!");
    return; }
...
window.setupRoomListener = function() {
    onValue(ref(db, 'rooms/' + window.roomCode), (snapshot) => {
        const data = snapshot.val();
        if (!data) { alert("ژوور هاتە گرتن!"); sessionStorage.removeItem('bingoOnlineSession'); window.location.reload(); return; }
...
window.switchToGameScreen = function() {
    ...
    else { document.getElementById('playerWaitMessage').style.display = 'block';
    document.getElementById('waitResetMessage').style.display = 'block'; }
}
...
window.addPlayerOffline = function() {
    ...
    if ([...new Set(numbers)].length !== numbers.length) { alert('تە ژمارەیەک دوو جاران یا نڤیسی. هیڤییە ژمارەیێن دووبارە ڕابکە!');
    return; }
    ...
    let dup = numbers.find(n => allUsed.includes(n)); if (dup !== undefined) { alert(`لێبۆڕینێ دخوازم، ژمارە (${dup}) پێشتر ژ لایێ کەسەکێ دی ڤە هاتییە هەلبژارتن!\nهیڤییە ژمارەیەکا دی بنڤیسە.`);
    return; }
...
window.deletePlayerOffline = function(id) {
    if (window.drawnNumbers.length > 0) return;
    if (window.players[id] && confirm(`ئایا تو یێ پشتڕاستی دڤێت یاریزان "${window.players[id].name}" ژێ ببەی؟`)) { delete window.players[id]; window.saveOfflineGameState(); window.renderPlayersTable();
    }
}
...
window.updateOfflineMenuState = function() {
    ...
    if (isStarted) {
        btnAdd.classList.add('btn-disabled'); btnAddOff.classList.add('btn-disabled');
        ...
        btnAdd.classList.remove('btn-disabled'); btnAddOff.classList.remove('btn-disabled');
...
window.changeTheme = function() { window.currentTheme = document.getElementById('themeSelect').value;
    window.applyTheme(); localStorage.setItem('bingoCustomTheme', window.currentTheme); }
window.applyTheme = function() { document.getElementById('mainBody').className = window.currentTheme !== 'classic' ?
    'theme-' + window.currentTheme : ''; }
window.changeLanguage = function() { window.selectedLang = document.getElementById('langSelect').value;
    localStorage.setItem('bingoCustomLang', window.selectedLang); }
window.toggleSoundMode = function() { const btn = document.getElementById('soundBtn');
    window.soundMode = (window.soundMode - 1 < 0) ? 2 : window.soundMode - 1;
    if (window.soundMode === 2) btn.innerText = "🔊 دەنگ + خواندن"; else if (window.soundMode === 1) btn.innerText = "🎵 بتنێ موزیک";
    else btn.innerText = "🔇 بێ دەنگ"; }
window.toggleAlert = function(type) { window.alertsEnabled[type] = !window.alertsEnabled[type];
...
window.togglePlayerMenu = function() { const m = document.getElementById('playerMenuContent');
    m.style.display = (m.style.display === 'none' || m.style.display === '') ? 'block' : 'none';
}
window.toggleSubMenu = function(id) { if (id === 'addPlayerPanel' && window.drawnNumbers.length > 0) return;
    const p = document.getElementById(id); if (p.style.display === 'block') { p.style.display = 'none'; } else { document.querySelectorAll('.menu-panel').forEach(pan => pan.style.display='none');
    p.style.display = 'block'; } }
window.addNumberBox = function(containerId) { const cont = document.getElementById(containerId);
    const inp = document.createElement('input'); inp.type='number'; inp.className='num-input-box'; cont.appendChild(inp); }
window.showHistory = function() { const list = document.getElementById('historyList');
    list.innerHTML = ''; if(window.drawnNumbers.length === 0) { list.innerHTML = '<span style="color:#888; font-size:14px;">هێشتا چ ژمارە نەدەرکەفتینە!</span>';
    } else { window.drawnNumbers.forEach((num, idx) => { const s = document.createElement('span'); s.className = 'number-circle matched'; s.style.width = '30px'; s.style.height = '30px'; s.style.lineHeight = '30px'; s.style.fontSize = '14px'; s.innerText = num; s.title = `ڕاکێشانا ژمارە ${idx + 1}`; list.appendChild(s); });
    ...
    if ('speechSynthesis' in window) { 
        ...
        if (voices.length > 0) {
            ...
            if (!target && window.selectedLang === 'ku') {
                target = voices.find(v => v.lang.startsWith('ar')) ||
...
window.playSound = function(type) { if (window.soundMode === 0) return;
    if (!window.audioCtx) window.audioCtx = new (window.AudioContext || window.webkitAudioContext)(); if (window.audioCtx.state === 'suspended') window.audioCtx.resume(); const osc = window.audioCtx.createOscillator();
    const gain = window.audioCtx.createGain(); osc.connect(gain); gain.connect(window.audioCtx.destination); if (type === 'draw') { osc.type = 'sine'; osc.frequency.setValueAtTime(800, window.audioCtx.currentTime); gain.gain.setValueAtTime(0.05, window.audioCtx.currentTime);
    gain.gain.exponentialRampToValueAtTime(0.001, window.audioCtx.currentTime + 0.1); osc.start(); osc.stop(window.audioCtx.currentTime + 0.1); } else if (type === 'safe') { osc.type = 'triangle'; osc.frequency.setValueAtTime(400, window.audioCtx.currentTime);
    osc.frequency.exponentialRampToValueAtTime(800, window.audioCtx.currentTime + 0.2); gain.gain.setValueAtTime(0.1, window.audioCtx.currentTime); gain.gain.exponentialRampToValueAtTime(0.001, window.audioCtx.currentTime + 0.3); osc.start(); osc.stop(window.audioCtx.currentTime + 0.3);
    } else if (type === 'lose') { osc.type = 'sawtooth'; osc.frequency.setValueAtTime(300, window.audioCtx.currentTime); osc.frequency.exponentialRampToValueAtTime(50, window.audioCtx.currentTime + 1.5); gain.gain.setValueAtTime(0.2, window.audioCtx.currentTime);
...
window.showCelebration = function(names) { if (!window.alertsEnabled.win) return;
...
window.initBoard = function() {
    ...
    for(let i = 1; i <= window.maxGameNumbers; i++) { let c = document.createElement('div'); c.className = 'board-cell';
    c.id = 'cell-' + i; c.innerText = i; if(window.drawnNumbers.includes(i)) c.classList.add('drawn'); b.appendChild(c);
    }
}
...
window.handleTouchStart = function(e) { if(window.gameMode==='online' || window.drawnNumbers.length>0) return;
    window.touchstartX = e.changedTouches[0].screenX; window.touchstartY = e.changedTouches[0].screenY; }
window.handleTouchEnd = function(e, id) { if(window.gameMode==='online' || window.drawnNumbers.length>0) return;
    let dx = Math.abs(e.changedTouches[0].screenX - window.touchstartX), dy = Math.abs(e.changedTouches[0].screenY - window.touchstartY); if (dx > 50 && dx > dy) window.deletePlayerOffline(id);
}

// نویکرنەڤەیا لیستا یاریزانان دگەل دوگمەیێن دەنگی
window.renderPlayersTable = function() {
    ...
    pArr.forEach(p => {
        ...
        let actionHtml = (window.gameMode==='offline'&&!isStarted) ? `<td><div style="display:flex;
    gap:4px; justify-content:center;"><button class="btn-edit-table" onclick="editPlayerOffline('${p.id}')">دەستکاری</button><button class="btn-delete-table" onclick="deletePlayerOffline('${p.id}')">🗑️</button></div></td>` : (window.gameMode === 'online' ? `<td>${muteBtnHtml}</td>` : '<td>-</td>');
...
{
    if (Object.keys(window.players).length < 2) { 
        window.stopAutoMode(); alert('ب لایەنی ڤە پێدڤییە ٢ یاریزان بهێنە تۆمارکرن بۆ دەستپێکرنا یاریێ!'); return; 
    }
...
window.toggleAutoDraw = function() {
    ...
    if (window.isAutoPlaying) { 
        ...
        let speedSecs = parseInt(document.getElementById('speedInput').value) ||
        3;
...
window.processDrawUI = function(drawn, isOnlineCall) {
    ...
    if (window.alertsEnabled.draw) {
        ...
        else { mt.innerHTML = `ژمارە <span class="highlight-num">${drawn}</span> دەرکەفت!<br><span style="font-size: 0.75em; color: #ccc;">(یا چ کەسان نینە)</span>`;
        mt.classList.remove('hit'); }
    }
    window.renderPlayersTable();
...
window.evaluateGameStateOffline = function() {
    ...
    if (remain.length === 1 && pArr.length > 1 && pArr.some(p => p.isSafe)) { if (!remain[0].isLoser) { hasLoser = true;
    loserObj = remain[0]; } } 
    ...
    if (newlySafe.length > 0 || hasLoser || hasGameEnd) {
        ...
        window.pendingAlertTimeout = setTimeout(() => {
            ...
            if (hasLoser) {
                loserObj.isLoser = 
                true; window.saveOfflineGameState(); window.renderPlayersTable();
                ...
                setTimeout(() => { 
                    if (document.getElementById('winnerModal').style.display !== 'flex' && window.alertsEnabled.lose) { document.getElementById('loserText').innerHTML = `یاری ب دوماهیک هات!<br><span style="color:var(--col-b); font-size: 0.5em;
                    text-shadow:none;">چ خوسارەت نینن</span>`; document.getElementById('winnerModal').style.display = 'flex'; } }, newlySafe.length > 0 ? 3500 : 0);
            }
...
window.evaluateGameStateLocal = function() {
    ...
    1 && playersArr.length > 1 && playersArr.some(p => p.isSafe)) {
        if (!remainingPlayers[0].isLoser) { hasLoser = true;
        loserObj = remainingPlayers[0]; if(window.isHost) updatesNeeded[`players/${loserObj.id}/isLoser`] = true; }
    ...
    if (newlySafePlayers.length > 0 || hasLoser || hasGameEnd) {
        ...
        window.pendingAlertTimeout = setTimeout(() => {
            ...
            if (hasLoser) {
                ...
                setTimeout(() => { if (document.getElementById('winnerModal').style.display 
                    !== 'flex' && window.alertsEnabled.lose) { document.getElementById('loserText').innerHTML = `یاری ب دوماهیک هات!<br><span style="color:var(--col-b); font-size: 0.5em; text-shadow:none;">چ خوسارەت نینن</span>`; document.getElementById('winnerModal').style.display = 'flex';
                } }, newlySafePlayers.length > 0 ? 3500 : 0);
            }
            window.pendingAlertTimeout = null;
        }, 2000); 
    }
...
window.resetGameDispatcher = async function() {
    if (window.drawnNumbers.length === 0) { document.getElementById('winnerModal').style.display = 'none';
    return; }
    if (confirm("ئایا تو دڤێتی یاریێ ژ نوی دەستپێبکەی؟\n(ژمارەیێن دەرکەفتی دێ هێنە ڕەشکرن، بەلێ یاریزان دێ مینن)")) {
        if (window.gameMode === 'online') {
            if (!window.isHost) return;
            let updates = { drawnNumbers: [], status: 'waiting' };
...
window.joinAgoraChannel = async function() {
    if (!window.agoraAppId || window.agoraAppId === "YOUR_AGORA_APP_ID_HERE") {
        console.warn("تکایە Agora App ID دابنێ د ناڤ کۆدی دا!");
        return;
    }
    try {
        window.agoraClient = AgoraRTC.createClient({ mode: "rtc", codec: "vp8" });
        // یاریزانان دێ ل سەر ناڤێ ژوورێ کۆک بن، و هەریەکێ ژمارەیەکا خۆ یا هەی
        await window.agoraClient.join(window.agoraAppId, window.roomCode, null, window.playerId);
        // دەمێ کەسەکێ دی مایکی ڤەدکەت
        window.agoraClient.on("user-published", async (user, mediaType) => {
            ...
            if (mediaType === "audio") {
                ...
                // ئەگەر مە ئەو یاریزانە بێدەنگ نەکر بیت، دەنگێ وی لێدە
        ...
        // دەمێ کەسەک مایکی دگریت
        window.agoraClient.on("user-unpublished", user => {
            if (window.remoteUsers[user.uid]) {
...
// ڤەکرن و گرتنا مایکرۆفۆنێ تە
window.toggleMyMic = async function() {
    if (!window.agoraClient) return;
    // ئەگەر چاتا دەنگی نەهاتیتە چالاککرن
    ...
    if (window.isMicMuted) {
        try {
            ...
            muteAllBtn.style.display = "flex";
        } catch (err) {
            alert("لێبۆڕینێ دخوازم، نەشیا مایکی ڤەکەت. پێدڤییە دەستویریێ (Allow) بدەیێ.");
        }
    } else {
        if (window.localAudioTrack) {
            await window.localAudioTrack.setMuted(true);
        }
        window.isMicMuted = true;
        micBtn.innerText = "🔇";
...
// گرتن یان ڤەکرنا دەنگێ یاریزانیەکێ دیارکری
...
// بێدەنگکرنا هەمی یاریزانان پێکڤە
window.toggleMuteAll = function() {
    ...
    if (window.isAllMuted) {
        muteAllBtn.innerText = "🔊";
        muteAllBtn.style.background = "#4caf50";
    } else {
        muteAllBtn.innerText = "🔕";
        muteAllBtn.style.background = "#ff9800";
    }
    window.renderPlayersTable();
};
